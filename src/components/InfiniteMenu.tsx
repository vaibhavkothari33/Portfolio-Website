...
const ATLAS_CELL_BASE_SIZE = 1024;
const BASE_CAMERA_DISTANCE = 6;
const DEFAULT_VIEW_SCALE = 1.9;
/** Padding between atlas cells to stop linear filtering from bleeding neighbors */
const ATLAS_GUTTER_PX = 8;
...
void main() {
    ...
    // Inset UVs so LINEAR sampling never reads the next atlas cell
    const float inset = 0.02;
    vec2 st = vec2(vUvs.x, 1.0 - vUvs.y);
    st = mix(vec2(inset), vec2(1.0 - inset), st);
...
type MenuItem = InfiniteMenuItem;
...
class InfiniteGridMenu {
  ...
  constructor(
    ...
    this.camera.position[2] = BASE_CAMERA_DISTANCE * scale;
    this.init(onInit);
  }
  ...
  private initTexture(): void {
    ...
    const cellSize = Math.round(ATLAS_CELL_BASE_SIZE * dpr);
    ...
    }).catch(err => console.error('[InfiniteMenu] texture atlas:', err));
  }
  ...
  private updateProjectionMatrix(): void {
    ...
    const height = this.SPHERE_RADIUS * 0.22;
  ...
  private onControlUpdate(deltaTime: number): void {
    ...
    let cameraTargetZ = BASE_CAMERA_DISTANCE * this.scaleFactor;
...
interface InfiniteMenuProps {
  items?: MenuItem[];
  /** Camera distance multiplier; higher = more zoomed out. Default ~1.45 */
  scale?: number;
  className?: string;
}
